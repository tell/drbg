...
inline std::string to_hex(const T& x, const std::string &delim = ",") { // {{{
    ...
    if (len > 0) {
        s << std::hex;
        s << "0x"
            << std::setw(2 * sizeof(typename T::value_type)) << std::setfill('0')
            << static_cast<uint_fast64_t>(x[0]);
        for (size_t i = 1; i < len; i++) {
            s << delim
                << "0x"
                << std::setw(2 * sizeof(typename T::value_type)) << std::setfill('0')
                << static_cast<uint_fast64_t>(x[i]);
        }
    }
    s << "}";
    return s.str();
...
public:
    ...
    keyed_ctr_drbg(const key_t &key, const ctx_t &ctx = {0}) : key_(key), ctx_(ctx) {}
    keyed_ctr_drbg() : keyed_ctr_drbg({0}) {}
    friend std::ostream& operator<<(std::ostream& o, const keyed_ctr_drbg &x) { // {{{
...
public:
    uint32_t getUInt32(const uint32_t ctr = 0) { // {{{
        seed_t seed;
        crypto_kdf_derive_from_key(seed.data(), seed.size(), ctr, ctx_.data(), key_.data());
        buff_t out(sizeof(uint32_t));
        randombytes_buf_deterministic(out.data(), sizeof(uint32_t), seed.data());
        uint32_t ret =
    ...
    uint64_t getUInt64(const uint64_t ctr = 0) { // {{{
        ...
        randombytes_buf_deterministic(out.data(), sizeof(uint64_t), seed.data());
        uint64_t ret =
